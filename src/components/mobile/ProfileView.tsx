...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  // 3. Simulate walking step to test distance updates
  const handleSimulateStep = () => {
    const nextCoords = simulateWalkingStep(userLocation.lat, userLocation.lng);
    const newLoc: UserGeoLocation = {
      ...userLocation,
      lat: nextCoords.lat,
      lng: nextCoords.lng,
          ...
          <div className="bg-neutral-950/80 rounded-2xl border border-neutral-800 p-3 space-y-2.5">
            ...
            <div className="grid grid-cols-2 gap-2 pt-1 border-t border-neutral-900">
              <button
                ...
                className="py-2 px-2.5 rounded-xl bg-gradient-to-r from-amber-500/20 to-amber-500/10 hover:bg-amber-500/30 border border-amber-500/40 text-amber-300 text-xs font-bold flex items-center justify-center gap-1.5 transition active:scale-98 disabled:opacity-50"
              ...
              <button
                type="button"
                id="profile-walk-step-btn"
                onClick={handleSimulateStep}
                className="py-2 px-2.5 rounded-xl bg-neutral-900 hover:bg-neutral-850 border border-neutral-800 text-neutral-200 text-xs font-semibold flex items-center justify-center gap-1.5 transition active:scale-98"
                title="Зсунути позицію на 80-100м пішки (тест переміщення між барами)"
              >
                <Footprints className="w-3.5 h-3.5 text-emerald-400" />
                <span>Крок пішки (+90м)</span>
              </button>
            </div>
          </div>
